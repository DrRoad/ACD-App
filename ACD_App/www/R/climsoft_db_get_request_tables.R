...
#'
#' @param db.type string. Type of database selected.
...
#' \code{\link{climsoft_db}} and 
#' \code{\link{climsoft-db_get_request_tables}}.
#'
################################################################################
...
getTables <- function(db.type){
  ...
  metService <- as.character(localSettings$metService)
  filename <- file.path(".","www","extdata", 
                        'db_conversion_updated_by_hand.csv')
  tables <- read.csv(filename)
  
  if( tolower(as.character(metService)) == tolower("DMS")){
    db.type.table <- paste0(db.type, ".table.dms")
    db.type.attr <- paste0(db.type, ".attr.dms")
