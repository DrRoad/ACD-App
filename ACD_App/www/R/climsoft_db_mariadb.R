################################################################################
#' @export climsoft_db_mariadb
#'
#' @title Connect to a Mariadb database and create options
#'
#' @description It connects to a selected mariadb database and displays the
...
climsoft_db_mariadb <- function(input, output, session){
  ##############################################################################
  ...
  dns_id <- (grep("mariadb", tolower(dns_prev)))
  dns <<- dns_prev[dns_id]
  ...
  observeEvent(input[[loginNew]],{
    ...
    observe({
      if ((exists("channel2")) && (!is.null(channel2)) && (channel2 != -1) &&
        ...
        # mainPanel - uiOutputs
        print("log: Connected to Mariadb")
        ...
        uiExport <<- paste0("uiExport_", uid2)
        ...
        observe({
          ...
          if(length(id0)!=0){
            ...
            if (df.users$Plots[id0] == T){
              output <- climsoft_db_plots(input, output, session)
            }
            ...
            if (df.users$RClimDex[id0] == T){
              #output <- climsoft_db_rclimdex(input, output, session)
            }
