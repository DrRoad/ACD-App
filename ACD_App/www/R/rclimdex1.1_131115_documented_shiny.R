#' @title Climate Indices Calculation software
#' @author Yujun Ouyang,Mar,2004, rewritten by Yang Feng, July 2004
#'
#' @description The RClimDex provides a friendly graphical user interface
#' to compute all 27 core indices . It also conducts simple quality control
#' on the input daily data. It has been developed and maintained by
#' Xuebin Zhang and Yang Feng at Climate Research Division (Canada).
#'
#' @details A description of the indices that can be calculated can be
#' found in the web:
#' \url{http://etccdi.pacificclimate.org/list_27_indices.shtml}
#' More information in \url{http://etccdi.pacificclimate.org/software.shtml}
#'
#' @note Code updated by K. Eiselt and W. Koertke (2015)
#'
#' -- modified, 2015-03-20 -- (K.Eiselt)
#' Changes in plotx():
#' colo="red1" oder , colo="blue1"
#' weitere Veraenderungen siehe ab Zeie 2529 (plotx()-Initialisierung)
#######--------------#######
#'
#' -- modified, 2013-11-15 --
#' in exceedance() function,
#' changed denominator for base period from 29 to (ys-1)
...
RClimDex2 <- function(){
  ...
  ###############################################################
  # CHECK R VERSION
  mysort<- if(getRversion()<='2.4.1') function(x,decreasing){
    .Internal(sort(x,decreasing=decreasing))
  }else
    function(x,decreasing){
      sort.int(x,decreasing=decreasing)
    }

  ###############################################################
  # WINDOW - DEFINITION OF THE SET PARAMETER VALUES
  ...
  pplotts<-function(var="prcp",type="h",tit=NULL){
    ...
    for(i in seq(years,yeare,10)){
      ...
      plot(1:length(ttmp),ttmp,type=type,col="blue1",xlab="",ylab="",xaxt="n",xlim=c(1,3660),ylim=c(ymin,ymax))
      abline(h=0)
      tt<-seq(1,length(ttmp))
      tt<-tt[is.na(ttmp)==T]
      axis(side=1,at=at,labels=c(i:(i+9)))
      for(k in 1:10) abline(v=at[k],col="yellow")
      lines(tt,rep(0,length(tt)),type="p",col="red1")
  ...
  ###########################################################
  # INDICES - GROWING SEASON LENGTH (GSL)
  ind143gsl<-function(){
    ...
    ####
    plotx(b[,1],b[,"gsl"],main=paste("Growing Season Length (GSL)",ofilename,sep="   "),xlab="Year",ylab="GSL", colo="red1")
    dev.off()
  }
  ...
  qcontrol<-function(){
    ...
    if(length(ttmp)>30){
      ...
      lines(density(ttmp,bw=0.2,from=1),col="red1")
    }
    ...
    print(paste("before calculating, crt=", crt))
    ...
    if (dim(o1data)[1] > 0){
      ...
      # to open log file directly
      ReturnVal <- tkmessageBox(message=paste("Outliers found, please check the log file: ",
    ...
    returnVal <- tkmessageBox(message=paste("If you have checked data (", namcal,
                                            "), continue with Indices Calculation. ",
                                            "\n", "Do you want to open indcal.csv file? "
                                            ,sep=""), type ="yesno")
    if (tclvalue(returnVal) == "yes"){
      system2("open", namcal)
    }
  ...
  ###########################################################
  # GET THE FILE WITH THE DATA & REESTRUCTURE THE DATA
  getfile<-function() {
    if (exists("shiny.station.id")){ #out of the shiny server func
      name <- file.path(path.expand("~"), FormatID(shiny.station.id),
                        paste(FormatID(shiny.station.id), ".txt", sep=""))
    } else {
    ...
    # Create folders (if the path is short)
    if(length(outdirtmp)<=2){
      ...
      # Create folders (if the path is long)
      outdir<-outdirtmp[1]
    ...
    # Create matrix (ddd = 365 days; dddl = 3666 days)
    ddd<-matrix(NA,365,6)
    ...
    # Include the month and day in the "ddd" matrix
    ...
    # Include the month and day in the "dddl" matrix
    ...
    # Include the years in the "ddd" matrix
  ...
  ###########################################################
  # INDICES (1) - WARM SPELL DURATION (WSPI)
  hwfi<-function(){
    if (flag==T) return()
    ...
    plotx(hwfi[,1],hwfi[,2], main=paste("Warm Spell Duration Indicator (WSDI)",ofilename,sep="   "),ylab="WSDI",xlab="Year", colo="red1")
    dev.off()
  } # end of hwfi function

  ###########################################################
  # INDICES (1) - COLD SPELL DURATION (CSPI)
  cwdi<-function(){
    if (flag==T) return()
    ...
    jpeg(nam2,width=1024,height=768)
    plotx(cwdi[,1],cwdi[,2],main=paste("Cold Spell Duration Indicator (CSDI)",ofilename,sep="   "),ylab="CSDI",xlab="Year", colo="red1")
    dev.off()
  } # end of cwdi function

  ###########################################################
  # INDICES (1) - VERY WET DAYS (R95p)
  r95ptot<-function(){
    ...
    plotx(dp[,1],dp[,"r95p"],main=paste("Very wet days (R95p)",ofilename,sep="   "),xlab="Year",ylab="R95p", colo="blue1")
    dev.off()
    nam5<-paste(outjpgdir,paste(ofilename,"_R99p.jpg",sep=""),sep="/")
    jpeg(nam5,width=1024,height=768)
    plotx(dp[,1],dp[,"r99p"],main=paste("Extremly wet days (R99p)",ofilename,sep="   "),xlab="Year",ylab="R99p", colo="blue1")
    dev.off()
    nam6<-paste(outjpgdir,paste(ofilename,"_PRCPTOT.jpg",sep=""),sep="/")
    jpeg(nam6,width=1024,height=768)
    plotx(dp[,1],dp[,"prcptot"],main=paste("Annual total wet-day precipitation (PRCPTOT)",ofilename,sep="   "),xlab="Year",ylab="PRCPTOT", colo="blue1")
    dev.off()
  } # end of function r95ptot

  ###########################################################
  # INDICES (1) - VERY HEAVY PRECIPITATION DAYS (R20mm)
  daysprcp20<-function(){
    ys<-yeare-years+1
    R20<-rep(0,ys)
    ...
    jpeg(nam2,width=1024,height=768)
    plotx(target[,1],target[,2],main=paste("Very heavy precipitation days (R20mm)",ofilename,sep="   "),xlab="Year",ylab="R20mm", colo="blue1")
    dev.off()
  }

  ###########################################################
  # INDICES (1) - NUMBER OF DAYS EXCEEDING A GIVEN THRESHOLD (Rnn)
  daysprcpn<-function(){
    ys<-yeare-years+1
    Rnn<-rep(0,ys)
    yearss<-c(years:yeare)
    ...
    jpeg(nam2,width=1024,height=768)
    plotx(target[,1],target[,2],main=paste("Days above ",as.character(nn),"mm (R",as.character(nn),"mm)  ",ofilename,sep=""),xlab="Year",ylab="Rnnmm", colo="blue1")
    dev.off()
  }
  ###########################################################
  # INDICES (4) - COLD NIGHTS, COLD DAYS, WARM NIGHTS, WARM DAYS
  ...
  ###########################################################
  # INDICES (1) - DIURNAL TEMPERATURE RANGE (DTR)
  dtr<-function(){# day temperature range(monthly average)
    ...
    jpeg(nam2,width=1024,height=768)
    plotx(ofile[,1],ofile[,14],main=paste("Diurnal temperature range (DTR)",ofilename,sep="   "),xlab="Year",ylab="DTR", colo="red1")
    dev.off()
  } # end of dtr

  ###########################################################
  # INDICES (1) - HEAVY PRECIPITATION DAYS (R10mm)
  daysprcp10<-function(){
    ys<-yeare-years+1
    R10<-rep(0,ys)
    yearss<-c(years:yeare)
    ...
    jpeg(nam2,width=1024,height=768)
    plotx(target[,1],target[,2],main=paste("Heavy precipitation days (R10mm)",ofilename,sep="   "),xlab="Year",ylab="R10mm", colo="blue1")
    dev.off()
  }

  ###########################################################
  # INDICES (4) - SU, FD, TR, ID
  extremedays<-function(opt=0){
    ...
    for(i in 1:4){
      if(opt==0){
        if (ylab[i]=="SU25") main<-"Summer days (SU25)"
        if (ylab[i]=="ID0") main<-"Ice days (ID0)"
        if (ylab[i]=="TR20") main<-"Tropical nights (TR20)"
        if (ylab[i]=="FD0") main<-"Frost days (FD0)"
      }else{
        if (ylab[i]==paste("SU",as.character(euu),sep="")) {main<-(paste("Summer days (SU",as.character(euu),")",sep=""))}
        if (ylab[i]==paste("ID",as.character(eul),sep="")) {main<-(paste("Ice days (ID",as.character(eul),")",sep=""))}
        if (ylab[i]==paste("TR",as.character(elu),sep="")) {main<-paste("Tropical nights (TR",as.character(elu),")",sep="")}
        if (ylab[i]==paste("FD",as.character(ell),sep="")) {main<-paste("Frost days (FD",as.character(ell),")",sep="")}}

      title1[i]<-paste(main,ofilename,sep="   ")
      jpeg(file=namp[i],width=1024,height=768)
      plotx(tclext[,1],tclext[,i+1],main=title1[i],ylab=ylab[i],xlab="Year", colo="red1")
      dev.off()
    }
  }

  ###########################################################
  # INDICES (4) - TX10p, TX90p, TN10p, TN90p
  exceedance<-function(){
    if (flag==T) return()
    a<-1:365
    ...
    for(i in c("tx10p","tx90p","tn10p","tn90p")){
      ...
      jpeg(file=nam2,width=1024,height=768)
      if (i=="tx10p") {main<-paste("Cool days (TX10p)",ofilename,sep="   ")}
      if (i=="tx90p") {main<-paste("Warm days (TX90p)",ofilename,sep="   ")}
      if (i=="tn10p") {main<-paste("Cool nights (TN10p)",ofilename,sep="   ")}
      if (i=="tn90p") {main<-paste("Warm nights (TN90p)",ofilename,sep="   ")}
      plotx(ofile[,1],ofile[,14],main=main,ylab=toupper(i),xlab="Year", colo="red1")
      dev.off()
    }
  }

  ###########################################################
  # INDICES - CONSECUTIVE DRY DAYS (CDD)
  index641cdd<-function(){
    ...
    jpeg(nam2,width=1024,height=768)
    plotx(target[,1],target[,2],main=paste("Consecutive dry days (CDD)",ofilename,sep="   "),xlab="Year",ylab="CDD", colo="blue1")
    dev.off()
  }

  ###########################################################
  # INDICES - CONSECUTIVE WET DAYS (CWD)
  index641cwd<-function(){
    ...
    jpeg(nam2,width=1024,height=768)
    plotx(target[,1],target[,2],main=paste("Consecutive wet days (CWD)",ofilename,sep="   "),xlab="Year",ylab="CWD", colo="blue1")
    dev.off()
  }

  ###########################################################
  # INDICES - MAX 1 DAY PRECIPITATION (RX1day)
  rx1d<-function(){
    len<-yeare-years+1
    aa1<-matrix(NA,12*len,3)
    ...
    jpeg(nam2,width=1024,height=768)
    plotx(ofile[,1],ofile[,14],main=paste("Max 1-day precipitation (RX1day)",ofilename,sep="   "),xlab="Year",ylab="RX1day", colo="blue1")
    dev.off()
  }# end of rx1d()

  ###########################################################
  # INDICES - MAX 1 DAY PRECIPITATION (RX5day)
  rx5d<-function(){
    a2<-c(0,0,0,0)
    a1<-dd[,"prcp"]
    a1<-append(a2,a1)
    ...
    jpeg(nam2,width=1024,height=768)
    plotx(ofile[,1],ofile[,14],main=paste("Max 5-day precipitation (RX5day)",ofilename,sep="   "),xlab="Year",ylab="RX5day", colo="blue1")
    dev.off()
  }# end of rx5d()

  ###########################################################
  # INDICES - TX10p, TX90p, TN10p, TN90p
  extremedaytem<-function(){
    ...
    for(i in c("Max Tmax (TXx)","Min Tmax (TXn)","Max Tmin (TNx)","Min Tmin (TNn)")){
      if (i=="Max Tmax (TXx)") {ii<-"_TXx.csv";ij<-"_TXx.jpg";kk<-3;ik<-1;ki<-3}# ik=1, take max as yearly record
      if (i=="Min Tmax (TXn)") {ii<-"_TXn.csv";ij<-"_TXn.jpg";kk<-4;ik<-0;ki<-3}# ik=0, take min as yearly record
      if (i=="Max Tmin (TNx)") {ii<-"_TNx.csv";ij<-"_TNx.jpg";kk<-5;ik<-1;ki<-4}# ki=3, take TMAX annual missing values
      if (i=="Min Tmin (TNn)") {ii<-"_TNn.csv";ij<-"_TNn.jpg";kk<-6;ik<-0;ki<-4}# ki=4, take TMIN annual missing values
      ...
      jpeg(nam2,width=1024,height=768)
      plotx(ofile[,1],ofile[,14],main=paste((i),ofilename,sep="   "),xlab="Year",ylab=(paste(toupper(substr(i,11,12)),substr(i,13,13),sep="")), colo="red1")
      dev.off()
      #toupper(i)
    }
  }
  ...
  ###########################################################
  # INDICES - SIMPLE DAILY INTENSITY (SDI)
  index646<-function(){
    ys=yeare-years+1
    ...
    jpeg(nam2,width=1024,height=768)
    plotx(b[,1],b[,2],main=paste("Simple daily intensity (SDII)",ofilename,sep="   "),xlab="Year",ylab="SDII", colo="blue1")
    dev.off()
  }

  ###########################################################
  # WINDOW - CALCULATING CLIMATE INDICES WINDOW
  main1<-function(){
    main<-tktoplevel()
    tkfocus(main)
...
#       tkgrid(textlabel0)
      ...
      #####
      RClimCompleted <<- tkmessageBox(message=paste("Indices calculation completed. ",
                                     "Plots are in: ",outjpgdir,sep=" "))
      #####
  ...
  ###########################################################
  # PLOT FUNCTION
  # Veraenderung in function():
  # , colo="red1"
  plotx <- function (x,y,main="",xlab="",ylab="", colo="red1"){
    op<-par(no.readonly=T)
    #par(font.axis=2, font.lab=2, font.main=2, font.sub=2)
    par(cex=2,mgp=c(2.5,.5,0), las=1)
    ## Veraenderung in plot():
    #, xaxt="n", cex.axis=0.7, col=colo
    plot(x,y,xlab=xlab,ylab=ylab,type="b", xaxt="n", cex.axis=0.7, col=colo)
    #####-----VON KAI BEGINN-----#####
    axis(1, at=seq(1850, 2015, 5), labels=seq(1850, 2015, 5), cex.axis=0.7)
    if(x[1] %% 2 == 0){
      bottom <- x[1]
    }else{
      bottom <- x[1] + 1
    }
    abline(v=seq(bottom,x[length(x)],2), col="gray", lty=2)
    if((max(y, na.rm=T) - min(y, na.rm=T)) >= 300){
      sequence <- seq(-1000, 1000, 50)
    }else if((max(y, na.rm=T) - min(y, na.rm=T)) >= 150){
      sequence <- seq(-1000, 1000, 20)
    }else if((max(y, na.rm=T) - min(y, na.rm=T)) >= 50){
      sequence <- seq(-1000, 1000, 5)
    }else if((max(y, na.rm=T) - min(y, na.rm=T)) >= 10){
      sequence <- seq(-1000, 1000, 2)
    }else if((max(y, na.rm=T) - min(y, na.rm=T)) >= 5){
      sequence <- seq(-1000, 1000, 1)
    }else{
      sequence <- seq(-100, 100, 0.5)
    }
    abline(h=sequence, col="gray", lty=2)
    #####-----VON KAI ENDE-----#####
    fit<-lsfit(x,y)
  ...
  startss<-function(){
    ...
    # Buttons
    if (exists("shiny.station.id")){
      start.but<-tkbutton(start1,text="Run Quality Control",command=getfile,width=30)
    } else {
  ...
  ###########################################################
  # POP UP RClimDex START WINDOW
  startss()

  ###########################################################
  # RETURN
  # The function returns the tcltk object so that it can be
  # called afterwards to close the RClimDex main Window
  return(start1)
}
